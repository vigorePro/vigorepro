...
function IAContent() {
  ...
  useEffect(() => {
    async function carregarEstab() {
      ...
      if (!session) return
      const { data: perfil } = await supabase.from('estabelecimentos').select('id').eq('user_id', session.user.id).single()
  ...
  const fetchDados = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const { data: msgs } = await supabase.from('conversas_ia').select('telefone, content, criado_em').eq('estabelecimento_id', estabelecimentoId).order('criado_em', { ascending: false })
      if (msgs) {
        ...
        msgs.forEach((m: { telefone: string, content: string, criado_em: string }) => {
          if (!map.has(m.telefone)) map.set(m.telefone, { total: 0, ultima: '', criado_em: m.criado_em })
        ...
        lista.forEach(c => { if (precisaHumano(c.ultima_mensagem)) novasPiscando.add(c.telefone) })
  ...
  useEffect(() => {
    if (!estabelecimentoId) return
    fetchDados()
    const channel = supabase.channel('ia-' + estabelecimentoId).on('postgres_changes', { event: '*', schema: 'public', table: 'conversas_ia', filter: 'estabelecimento_id=eq.' + estabelecimentoId }, () => fetchDados()).subscribe()
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1100, fontFamily: 'Mulish, sans-serif', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: '#1a1a1a', borderRadius: 10, padding: 4, width: 'fit-content' }}>
        {(['painel', 'teste'] as const).map(aba => (
          <button key={aba} onClick={() => setAbaAtiva(aba)} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13, fontFamily: 'Mulish, sans-serif', background: abaAtiva === aba ? '#ef4239' : 'transparent', color: abaAtiva === aba ? '#fff' : '#9ca3af', transition: 'all 0.2s' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                  <div style={{ background: getCanalTelefone(conversaAberta.telefone).cor + '22', borderRadius: 20, padding: '1px 8px', fontSize: 11, color: getCanalTelefone(conversaAberta.telefone).cor, fontWeight: 600 }}>{getCanalTelefone(conversaAberta.telefone).label}</div>
