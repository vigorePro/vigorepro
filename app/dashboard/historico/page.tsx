...
function HistoricoContent() {
  ...
  // KPIs
  const totalReceita = pedidos.filter(p => p.status !== 'cancelado').reduce((a, p) => a + (p.valor_total || 0), 0)
  ...
  return (
    <div style={{ fontFamily: 'Mulish, sans-serif', padding: 24, color: '#e6e6e6' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: '#fff' }}>HistÃ³rico de Vendas</h1>
          <p style={{ margin: '4px 0 0', fontSize: 14, color: '#888' }}>{total} pedido(s) no perÃ­odo</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: 'Ticket MÃ©dio', valor: 'R$ ' + ticketMedio.toFixed(2).replace('.', ','), icon: <TrendingUp size={20} />, cor: '#f59e0b' },
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #292929' }}>
              {['#', 'Data/Hora', 'Tipo', 'Status', 'Cliente/Mesa', 'Itens', 'Desconto', 'Total', 'Pagamento', 'AÃ§Ãµes'].map(col => (
          ...
          <tbody>
            {carregando ? (
              ...
              <tr><td colSpan={10} style={{ padding: 60, textAlign: 'center', color: '#555' }}>
                ...
                Nenhum pedido no perÃ­odo selecionado
              </td></tr>
            ) : pedidos.map((p, i) => (
              <>
                <tr key={p.id} style={{ borderBottom: '1px solid #1e1e1e', cursor: 'pointer' }}
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#888' }}>
                      {tipoIcon[p.tipo || 'balcao']} {p.tipo || 'BalcÃ£o'}
                    </span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                  ...
                  <td style={{ padding: '10px 14px', fontSize: 13, color: '#e6e6e6' }}>{p.cliente || (p.mesa ? 'Mesa ' + p.mesa : 'BalcÃ£o')}</td>
                  <td style={{ padding: '10px 14px', fontSize: 13, color: '#888', textAlign: 'center' }}>{p.itens?.length || 0}</td>
                  <td style={{ padding: '10px 14px', fontSize: 13, color: '#ef4444' }}>{p.desconto ? '- R$ ' + p.desconto.toFixed(2).replace('.', ',') : 'â'}</td>
                  <td style={{ padding: '10px 14px', fontSize: 14, fontWeight: 700, color: p.status === 'cancelado' ? '#ef4444' : '#22c55e' }}>R$ {(p.valor_total || 0).toFixed(2).replace('.', ',')}</td>
                  <td style={{ padding: '10px 14px', fontSize: 12, color: '#888', textTransform: 'capitalize' }}>{p.forma_pagamento || 'â'}</td>
                  <td style={{ padding: '10px 14px' }}>
                      ...
                      <div style={{ borderLeft: '2px solid #292929', paddingLeft: 16 }}>
                        {p.itens.map((item: any, idx: number) => (
                          <div key={idx} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '4px 0', fontSize: 13 }}>
                            ...
                            {item.observacao && <span style={{ color: '#f59e0b', fontSize: 12, fontStyle: 'italic' }}>â  {item.observacao}</span>}
      ...
      {totalPaginas > 1 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, color: '#888' }}>
          <span>Mostrando {(pagina - 1) * POR_PAGINA + 1}â{Math.min(pagina * POR_PAGINA, total)} de {total}</span>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => setPagina(p => Math.max(1, p - 1))} disabled={pagina === 1} style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #292929', background: '#1a1a1a', color: pagina === 1 ? '#444' : '#888', cursor: pagina === 1 ? 'not-allowed' : 'pointer', fontFamily: 'Mulish, sans-serif' }}>â Anterior</button>
            <span style={{ padding: '6px 14px', color: '#ef4239', fontWeight: 700 }}>{pagina} / {totalPaginas}</span>
            <button onClick={() => setPagina(p => Math.min(totalPaginas, p + 1))} disabled={pagina === totalPaginas} style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #292929', background: '#1a1a1a', color: pagina === totalPaginas ? '#444' : '#888', cursor: pagina === totalPaginas ? 'not-allowed' : 'pointer', fontFamily: 'Mulish, sans-serif' }}>PrÃ³ximo â</button>
          </div>
        </div>
      )}
    </div>
