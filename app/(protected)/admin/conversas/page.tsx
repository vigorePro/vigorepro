...
// Gera um ID simples: 3 dígitos + 1 letra (ex: 042B)
...
function ConversasContent() {
  ...
  const abrirConversa = async (telefone: string) => {
        ...
        // Se o cliente existe mas ainda não tem codigo, gera e salva um agora
  ...
  if (loading) return <div className="flex h-screen items-center justify-center text-gray-500">Carregando...</div>div>;
    if (erro) return <div className="flex h-screen items-center justify-center text-red-600">{erro}</div>div>;
            ...
            {/* Lista de contatos */}
                <div className="w-80 bg-white border-r flex flex-col flex-shrink-0">
                        <div className="p-4 bg-green-600 text-white">
                                  <h1 className="text-xl font-bold">Conversas</h1>h1>
                                  <p className="text-sm text-green-100">{contatos.length} contatos</p>p>
                        </div>div>
                        ...
                        <div className="p-4 text-center text-gray-400 text-sm">Nenhuma conversa encontrada</div>div>
                                      ...
                                      <div className="flex items-center gap-3">
                                                      ...
                                                      <div className="flex-1 min-w-0">
                                                                        <div className="flex justify-between items-baseline">
                                                                                            <span className="font-medium text-sm truncate">{contato.nome || contato.telefone}</span>span>
                                                                                            <span className="text-xs text-gray-400 ml-2 flex-shrink-0">{formatarData(contato.ultima_interacao)}</span>span>
                                                                        </div>div>
                                                                        <p className="text-xs text-gray-500 truncate mt-0.5">{contato.ultima_mensagem}</p>p>
                                                      </div>div>
                                      </div>div>
                        </button>button>
                      ))}
                ...
                <div className="flex-1 flex flex-col min-w-0">
                  {!contatoSelecionado ? (
                      <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                                  ...
                                  <p className="text-lg">Selecione uma conversa</p>p>
                                  <p className="text-sm">Escolha um contato na lista ao lado</p>p>
                      </div>div>
                    ) : (
                      <>
                        {/* Header */}
                                  <div className="px-4 py-3 bg-white border-b flex items-center gap-3 shadow-sm">
                                                ...
                                                <div className="flex-1">
                                                                <p className="font-semibold">{contatoAtual?.nome || 'Sem nome'}</p>p>
                                                                <p className="text-xs text-gray-500">
                                                                  {contatoAtual?.codigo ? (
                                            <span className="font-mono bg-gray-100 px-1.5 py-0.5 rounded text-gray-600 mr-1">#{contatoAtual.codigo}</span>span>
                                          ) : null}
                        ...
                        {abaAtiva === 'conversa' && (
                                      <>
                                                      <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50">
                                                        {loadingMensagens ? (
                                                            <div className="text-center text-gray-400 pt-10">Carregando mensagens...</div>div>
                                                          ) : mensagens.length === 0 ? (
                                                            <div className="text-center text-gray-400 pt-10">Nenhuma mensagem</div>div>
                                                          ) : mensagens.map((msg) => (
                                                            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                                                                  <div className={`max-w-xs lg:max-w-md px-3 py-2 rounded-lg text-sm shadow-sm ${msg.role === 'user' ? 'bg-green-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none'}`}>
                                                                                                          ...
                                                                                                          <p className={`text-xs mt-1 text-right ${msg.role === 'user' ? 'text-green-100' : 'text-gray-400'}`}>{formatarHora(msg.criado_em)}</p>p>
                                                                                    </div>div>
                                                            </div>div>
                                                          ))}
                        ...
                        {abaAtiva === 'perfil' && (
                                      <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
                                        {!perfil ? (
                                                          <div className="text-center text-gray-400 pt-10">Carregando perfil...</div>div>
                                                        ) : (
                                                          <div className="max-w-2xl mx-auto space-y-4">
                                                            ...
                                                            {/* Card de dados do contato */}
                                                                              <div className="bg-white rounded-xl p-4 shadow-sm">
                                                                                                    <div className="flex justify-between items-center mb-3">
                                                                                                                            <h3 className="font-semibold text-gray-700">📋 Dados do contato</h3>h3>
                                                                                                      {!editando && (
                                                                                ...
                                                                                {!editando ? (
                                                                                    ...
                                                                                    <div className="space-y-2">
                                                                                      {perfil.codigo && (
                                                                                                                  <div className="flex gap-2 items-center">
                                                                                                                                                <span className="text-sm text-gray-400 w-20">ID</span>span>
                                                                                                                                                <span className="font-mono text-sm font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded">#{perfil.codigo}</span>span>
                                                                                                                    </div>div>
                                                                                                              )}
                                                                                                              <div className="flex gap-2">
                                                                                                                                          <span className="text-sm text-gray-400 w-20">Nome</span>span>
                                                                                                                                          <span className="text-sm text-gray-700 font-medium">{perfil.nome || <span className="text-gray-400 italic">Sem nome</span>span>}</span>span>
                                                                                                                </div>div>
                                                                                      </div>div>
                                                                                  ) : (
                                                                                    // Modo edição
                                                                                    <div className="space-y-3">
                                                                                      {perfil.codigo && (
                                                                                                              ...
                                                                                                              <div>
                                                                                                                                          <label className="block text-xs text-gray-500 mb-1">Nome</label>label>
                                                            ...
                                                            {/* Métricas */}
                                                                              <div className="grid grid-cols-3 gap-3">
                                                                                                    <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                                                                                                                            <p className="text-2xl font-bold text-green-600">{perfil.total_pedidos}</p>p>
                                                                                                                            <p className="text-xs text-gray-500 mt-1">Pedidos</p>p>
                                                                                                      </div>div>
                                                                                                    <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                                                                                                                            <p className="text-2xl font-bold text-green-600">R$ {perfil.valor_total_gasto.toFixed(2)}</p>p>
                                                                                                                            <p className="text-xs text-gray-500 mt-1">Total gasto</p>p>
                                                                                                      </div>div>
                                                                                                    <div className="bg-white rounded-xl p-4 text-center shadow-sm">
                                                                                                                            <p className="text-2xl font-bold text-green-600">R$ {perfil.ticket_medio.toFixed(2)}</p>p>
                                                                                                                            <p className="text-xs text-gray-500 mt-1">Ticket médio</p>p>
                                                                                                      </div>div>
                                                                              </div>div>
                                                          
                                                            {/* Top 3 produtos favoritos */}
                                                                              <div className="bg-white rounded-xl p-4 shadow-sm">
                                                                                                    <h3 className="font-semibold text-gray-700 mb-3">⭐ Produtos favoritos</h3>h3>
                                                                                {!perfil.produtos_preferidos || perfil.produtos_preferidos.length === 0 ? (
                                                                                    <p className="text-sm text-gray-400">Nenhum pedido registrado ainda</p>p>
                                                                                  ) : (
                                                                                    <div className="space-y-2">
                                                                                      {perfil.produtos_preferidos.slice(0, 3).map((produto, i) => (
                                                                                                                  <div key={i} className="flex items-center gap-3">
                                                                                                                                                ...
                                                                                                                                                <span className="text-sm text-gray-700">{produto}</span>span>
                                                                                                                    </div>div>
                                                                                                                ))}
                                                                                      </div>div>
                                                                                                    )}
                                                            ...
                                                            {/* Histórico de pedidos */}
                                                                              <div className="bg-white rounded-xl p-4 shadow-sm">
                                                                                                    <h3 className="font-semibold text-gray-700 mb-3">🛍️ Histórico de compras</h3>h3>
                                                                                {perfil.pedidos.length === 0 ? (
                                                                                    <p className="text-sm text-gray-400">Nenhum pedido encontrado</p>p>
                                                                                  ) : (
                                                                                    <div className="space-y-3">
                                                                                      {perfil.pedidos.map((pedido) => (
                                                                                                                  <div key={pedido.id} className="border rounded-lg p-3">
                                                                                                                                                <div className="flex justify-between items-start mb-2">
                                                                                                                                                                                <div>
                                                                                                                                                                                                                  <span className="font-medium text-sm">Pedido #{pedido.numero_pedido}</span>span>
                                                                                                                                                                                                                  <span className="text-xs text-gray-400 ml-2">{formatarDataCompleta(pedido.criado_em)}</span>span>
                                                                                                                                                                                                                </div>div>
                                                                                                                                                                                <div className="flex items-center gap-2">
                                                                                                                                                                                                                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusCor[pedido.status] || 'bg-gray-100 text-gray-600'}`}>
                                                                                                                                                                                                                                                      {pedido.status.replace('_', ' ')}
                                                                                                                                                                                                                                                    </span>span>
                                                                                                                                                                                                                  <span className="text-sm font-semibold text-green-600">R$ {pedido.valor_total.toFixed(2)}</span>span>
                                                                                                                                                                                                                </div>div>
                                                                                                                                                  </div>div>
                                                                                                                                                ...
                                                                                                                                                <p className="text-xs text-gray-400 mt-1 capitalize">{pedido.tipo_entrega}</p>p>
                                                                                                                    </div>div>
                                                                                                                ))}
                                                                                      </div>div>
                                                                                                    )}
...
export default function ConversasPage() {
    return (
          <Suspense fallback={<div className="flex h-screen items-center justify-center">Carregando...</div>div>}>
                <ConversasContent />
          </Suspense>Suspense>
        );
}</></></div>
