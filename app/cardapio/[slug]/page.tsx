...
if (carregando) return (
  <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f4f5f7', fontFamily: 'Jost, Mulish, sans-serif' }}>
...
if (!estabelecimento) return (
  <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f4f5f7' }}>
    <div style={{ textAlign: 'center', color: '#888' }}>
      <h2>Restaurante nao encontrado</h2>
      <p>O estabelecimento <strong>{slug}</strong> nao existe.</p>
    </div>
  </div>
)

const corBtn = corPrimaria

return (
  <div style={{ fontFamily: 'Jost, Mulish, sans-serif', background: '#f4f5f7', minHeight: '100vh' }}>
    <link href="https://fonts.googleapis.com/css2?family=Jost:wght@400;500;600;700;800&display=swap" rel="stylesheet" />

    {/* HEADER - BeeFood style */}
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {estabelecimento.logo_url ? (
            <img src={estabelecimento.logo_url} alt="logo" style={{ width: 46, height: 46, borderRadius: 10, objectFit: 'cover' }} />
          ) : (
            <div style={{ width: 46, height: 46, borderRadius: 10, background: corPrimaria, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 20 }}>
              {estabelecimento.nome.charAt(0)}
            </div>
          )}
          <div>
            <h1 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: '#111', lineHeight: 1.2 }}>{estabelecimento.nome}</h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: estabelecimento.aberto !== false ? '#22c55e' : '#ef4444' }} />
              <span style={{ fontSize: 12, color: estabelecimento.aberto !== false ? '#22c55e' : '#ef4444', fontWeight: 500 }}>
        ...
        <button onClick={() => setCarrinhoAberto(true)} style={{
          position: 'relative', background: corPrimaria, border: 'none', borderRadius: 10,
          padding: '10px 18px', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 14, fontFamily: 'inherit'
        }}>
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 20px 100px' }}>
      ...
      <div style={{ position: 'relative', margin: '16px 0 8px' }}>
        <Search size={16} color="#bbb" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)' }} />
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar no cardapio..."
          style={{ width: '100%', padding: '12px 14px 12px 42px', borderRadius: 12, border: '1.5px solid #e5e7eb', background: '#fff', fontSize: 14, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit', color: '#333', transition: 'border-color 0.15s' }} />
      </div>

      {/* CATEGORIAS - BeeFood pills style */}
      {categorias.length > 0 && (
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '4px 0 14px', scrollbarWidth: 'none' }}>
          {[{ id: 'todas', nome: 'Todos' }, ...categorias].map(cat => (
            <button key={cat.id} onClick={() => setCategoriaAtiva(cat.id)} style={{
              padding: '7px 18px', borderRadius: 999, border: categoriaAtiva === cat.id ? 'none' : '1.5px solid #e0e0e0',
              whiteSpace: 'nowrap', background: categoriaAtiva === cat.id ? corPrimaria : '#fff',
              color: categoriaAtiva === cat.id ? '#fff' : '#555',
              fontWeight: categoriaAtiva === cat.id ? 700 : 500, fontSize: 13, cursor: 'pointer',
              fontFamily: 'inherit', flexShrink: 0,
              boxShadow: categoriaAtiva === cat.id ? '0 2px 6px rgba(0,0,0,0.12)' : '0 1px 3px rgba(0,0,0,0.06)'
            }}>{cat.nome}</button>
          ))}
        </div>
      )}

      {/* PRODUTOS - BeeFood list card style */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {produtosFiltrados.map(produto => (
            <div key={produto.id} onClick={() => { setProdutoDetalhe(produto); setQtdItem(1); setObsItem('') }}
              style={{ background: '#fff', borderRadius: 14, padding: 16, cursor: 'pointer', display: 'flex', gap: 14, alignItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', transition: 'box-shadow 0.2s' }}
              onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 14px rgba(0,0,0,0.10)'}
              onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.boxShadow = '0 1px 4px rgba(0,0,0,0.06)'}
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ margin: '0 0 4px', fontWeight: 700, fontSize: 16, color: '#111', lineHeight: 1.3 }}>{produto.nome}</p>
                ...
                <span style={{ fontSize: 15, fontWeight: 700, color: corPrimaria }}>
                  R$ {(produto.preco_promocional || produto.preco).toFixed(2).replace('.', ',')}
                </span>
              </div>
              {produto.imagem_url && (
                <img src={produto.imagem_url} alt={produto.nome} style={{ width: 82, height: 82, borderRadius: 10, objectFit: 'cover', flexShrink: 0 }} />
              )}
        ...
        <div style={{ background: '#fff', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 860, maxHeight: '90vh', overflowY: 'auto', padding: 24, position: 'relative' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea value={obsItem} onChange={e => setObsItem(e.target.value)} placeholder="Ex: sem cebola, bem passado..."
              rows={2} style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #e0e0e0', fontSize: 14, outline: 'none', resize: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          </div>
        ...
        <div style={{ width: 380, background: '#fff', display: 'flex', flexDirection: 'column', height: '100vh' }}>
          ...
          {carrinho.length > 0 && (
            <div style={{ padding: 20, borderTop: '1px solid #eee' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 14, color: '#666' }}><span>Subtotal</span><span>R$ {totalCarrinho.toFixed(2).replace('.', ',')}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, fontSize: 14, color: '#666' }}><span>Entrega</span><span style={{ color: totalCarrinho >= 100 ? '#22c55e' : '#555' }}>{totalCarrinho < 50 ? 'R$ 10,00' : totalCarrinho < 100 ? 'R$ 5,00' : 'Grátis 🎉'}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, fontSize: 17, fontWeight: 800 }}><span>Total</span><span style={{ color: corPrimaria }}>R$ {(totalCarrinho + (totalCarrinho < 50 ? 10 : totalCarrinho < 100 ? 5 : 0)).toFixed(2).replace('.', ',')}</span></div>
              <button onClick={finalizarComMEL} style={{ width: '100%', padding: '14px 0', background: '#25D366', border: 'none', borderRadius: 10, color: '#fff', fontWeight: 700, fontSize: 16, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
              ...
              <p style={{ textAlign: 'center', fontSize: 12, color: '#aaa', margin: '8px 0 0' }}>Você será direcionado ao WhatsApp :)</p>
            </div>
          )}
        </div>
      </div>
    ...
    {pedidoFinalizado && (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: 40, maxWidth: 340, textAlign: 'center', fontFamily: 'inherit' }}>
          ...
          <button onClick={() => { setPedidoFinalizado(false); setChatAberto(false) }} style={{ padding: '12px 32px', background: corPrimaria, border: 'none', borderRadius: 10, color: '#fff', fontWeight: 700, fontSize: 15, cursor: 'pointer', fontFamily: 'inherit' }}>Fazer novo pedido</button>
        </div>
      </div>
    )}

    {/* CHAT MEL FLUTUANTE */}
    <div style={{ position: 'fixed', bottom: 24, right: 20, zIndex: 999, fontFamily: 'inherit' }}>
      {chatAberto && (
        <div style={{ position: 'absolute', bottom: 72, right: 0, width: 340, height: 480, background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '1px solid #eee' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {chatMensagens.map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
                ...
                <div style={{ maxWidth: '78%', padding: '9px 13px', borderRadius: m.role === 'user' ? '14px 14px 2px 14px' : '14px 14px 14px 2px', background: m.role === 'user' ? corPrimaria : '#f3f4f6', color: m.role === 'user' ? '#fff' : '#111', fontSize: 13, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{m.content}</div>
              </div>
            ))}
            {chatCarregando && (
          ...
          <div style={{ padding: '10px 12px', borderTop: '1px solid #f0f0f0', display: 'flex', gap: 8, flexShrink: 0 }}>
            <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && enviarMensagemChat()} placeholder="Digite sua mensagem..." style={{ flex: 1, border: '1px solid #e5e7eb', borderRadius: 20, padding: '9px 14px', fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
            <button onClick={() => enviarMensagemChat()} disabled={chatCarregando} style={{ width: 38, height: 38, borderRadius: '50%', background: chatCarregando ? '#ccc' : corPrimaria, border: 'none', cursor: chatCarregando ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Send size={15} color="#fff" />
            </button>
          </div>
        </div>
      )}
      <button onClick={() => setChatAberto(!chatAberto)} style={{ width: 58, height: 58, borderRadius: '50%', background: corPrimaria, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', position: 'relative' }}>
        {chatAberto ? <X size={24} color="#fff" /> : <MessageCircle size={24} color="#fff" />}
        {!chatAberto && chatMensagens.length > 1 && (
          <div style={{ position: 'absolute', top: 2, right: 2, width: 18, height: 18, background: '#ef4444', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#fff', fontWeight: 700 }}>{chatMensagens.filter(m => m.role === 'assistant').length}</div>
        )}
      </button>
    </div>
  </div>
