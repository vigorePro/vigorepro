...
function CozinhaContent() {
  ...
  useEffect(() => {
        if (!slug) return
        // Busca o estabelecimento_id pelo slug primeiro
                supabase
  ...
  useEffect(() => {
        ...
        buscarPedidos(estabelecimentoId)
  ...
  async function buscarPedidos(estabId: string) {
  ...
  async function avancarStatus(pedido: Pedido) {
        ...
        await supabase
          ...
          .eq('id', pedido.id)
        if (estabelecimentoId) buscarPedidos(estabelecimentoId)
  }
